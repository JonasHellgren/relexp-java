package core.plotting_core.plotting_2d;

import com.google.common.base.Preconditions;
import core.foundation.util.cond.ConditionalsUtil;
import core.foundation.util.math.MathUtil;
import core.foundation.util.math.MatrixListUtil;
...
import static core.plotting_core.base.shared.FormattedAsString.getFormattedAsString;
...
public class ManyLinesChartCreator {
    ...
    public static ManyLinesChartCreator of(PlotSettings settings, List<Double> xData) {
        ...
        ConditionalsUtil.executeIfTrue(settings.isDefinedSpaceBetweenXTicks(), () ->
    ...
    public XYChart create() {
        ...
        styler.setYAxisMin(MatrixListUtil.findMin(yData));
        styler.setYAxisMax(MatrixListUtil.findMax(yData));
        ...
        ConditionalsUtil.executeIfTrue(s.colorRangeSeries() != null, () ->
                styler.setSeriesColors(s.colorRangeSeries()));
        for (String name : names) {
            int i = names.indexOf(name);
            var series = chart.addSeries(name, xList, yData.get(i));
            ConditionalsUtil.executeIfFalse(s.showMarker(), () -> series.setMarker(SeriesMarkers.NONE));
        }
        return chart;
    }
    ...
    private void setAxisTicksFormatting(XYStyler styler) {
        if (settings.isDefinedSpaceBetweenXTicks()) {
            styler.setxAxisTickLabelsFormattingFunction(value ->
                    (MathUtil.isZero(value % settings.spaceBetweenXTicks())
